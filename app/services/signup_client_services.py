...
class SignupClient():
    ...
    def check_keys(self, json_data):
        for key in json_signup_client:
            try:
                json_data.get(json_data[key])
            except KeyError:
                self.can_register = False
                return {"specific_error": f"{key} is missing", "Use this sample JSON": f"{json_signup_client}"}
        self.can_register = True
        return json_data
    # Check empty values
    ...
    def check_if_the_email_is_written_correctly(self, json_data):
        ...
        if (re.search(regex, json_data['email'])):
            ...
            self.can_register = False
            return {"specific_error": f"O email está errado", "Use this sample JSON": f"{json_signup_client}"}
    ...
    def conditionally_validate_the_password(self, json_data):
        ...
        if len(json_data['password']) < 3:
            self.can_register = False
            return {"specific_error": "A senha precisa ter no mínimo 3 caracteres e 15 no máximo", "Use this sample JSON": f"{json_signup_client}"}
        return json_data
